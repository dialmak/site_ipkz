...
- [x] Відкрийте *Налаштування* на Вашому телефоні й підключіться до цієї Wi-Fi мережі `IPKZ-F2B1D4`.    
...
- [x] Зайти в системне меню (1) клацнувши :material-menu: 
- [x] Клацнути на знак напівмісяця :fontawesome-solid-moon: для зміни теми (2)
- [x] Клацнути :material-window-close: для виходу з системного меню (3)
</div>
1. ![Зайти в системне меню.](../assets/images/theme/theme-1.png) 
2. ![Зміна теми.](../assets/images/theme/theme-2.png) 
...
=== "Скріншот головної сторінки"
    ...
    ![Головна сторінка.](../assets/images/main/main.png){ width="150" }  

### Зміна режиму роботи { #select-mode data-toc-label='Зміна режиму' }

Відключити вихід або перейти в режим стабілізації та/або уставки напруги, струму та потенціалу:  

<div class="annotate" markdown>
- [x] В пункті *Вибір режиму* (1) потрібно клацнути :fontawesome-solid-angle-down:.
...
5. ![Встановлення уставки. Другий спосіб.](../assets/images/mode/mode5.png) 
...
!!! tip annotate "Важливо!"
    ...
    При введенні значення поза діапазону - це значення буде автоматично змінено до допустимоі межи.

### Встановлення дати та часу { #setting-datetime data-toc-label='Дата та час' }

Встановлення дати та часу в два кроки:
...
=== "Опис сторінки"
    ...
    - Налаштування ModBus RTU/TCP.   
    - Налаштування електролічильника.    
...
=== "Скріншот сторінки"
    ...
    ![Cторінка *Налаштування*.](../assets/images/settings/settings2.png){ width="200" }  

### Обмеження вихідного струму та напруги при аварії { #limits data-toc-label='Обмеження струму та напруги' }
...
Зміна обмеження вихідного струму та напруги при аварії:

- [x] Для зміни потрібно клацнути на поточне значення параметра та ввести нове значення й підтвердити кнопкою *ОК*.
...
### Налаштування ПІД регулятора { #pid data-toc-label='ПІД регулятор' }
...
Існує два режими роботи ПІД регулятора: *Стандартний режим* та *Ручний режим*.
...
Налаштування ПІД регулятора в декілька кроків:
...
!!! note "Рекомендація"
    Перед зміною коефіціентів ПІД регулятора для *Ручного режиму* [відключіть вихід](#select-mode), а після зміни їх перейдіть в потрібний [режим стабілізації.](#select-mode)

!!! warning "Зверніть увагу!"
    При введенні числа використовуйте ==крапку==, як десятковий роздільник.

!!! tip annotate "Важливо!"
    Допустимий діапазон значеннь коефіцієнтів ПІД регулятора. 
...
Для зміни бітрейта мережі RS-485:

<div class="annotate" markdown>
- [x] В пункті *Бітрейт RS-485 порта* (1) потрібно клацнути :fontawesome-solid-angle-down:.
...
Для зміни ID адреси ModBus RTU сервера в мережі RS-485:

<div class="annotate" markdown>
- [x] В пункті *ModBus ID адреса в мережі RS-485* (1) потрібно клацнути :fontawesome-solid-angle-down:.
...
Для зміни ID адреси ModBus TCP сервера в мережі GSM:

<div class="annotate" markdown>
- [x] В пункті *ModBus ID адреса в мережі GSM* (1) потрібно клацнути :fontawesome-solid-angle-down:.
- [x] Далі виберіть необхідну ModBus ID адресу (2) у меню, що з'явилося. 
</div>
1. ![Зміна ModBus ID адреси в мережі GSM.](../assets/images/modbus/modbus5.png) 
2. ![Вибір новоі ModBus ID адреси в мережі GSM.](../assets/images/modbus/modbus6.png) 
...
!!! tip annotate "Важливо!"
    ...
    - ID адреси ModBus RTU сервера кожного перетворювача в одній мережі RS-485 має бути ==унікальна== й встановлена індивідуально в діапазоні від 1 до 247. Заводське значення 1.
...
Детальніше про [віддалене керування по протоколу Modbus RTU/TCP.](.././modbus/modbus.md)   

### Налаштування електролічильника { #meter data-toc-label='Eлектролічильник' }

### Вибір мови інтерфейсу { #language data-toc-label='Мова інтерфейсу' }

### Перехід на сторінку *Про перетворювач* { #go-about data-toc-label='До інформації про прилад' }
